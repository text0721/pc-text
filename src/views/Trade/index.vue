    ...
    <div class="content">
      ...
      >
        <span
          :class="{
            username: true,
            selected: userAddress.id === defaultselectId,
          }"
          @click="defaultselectId = userAddress.id"
...
export default {
  ...
  computed: {
    ...
    selectAddress() {
      ...
      //请求是异步的，先判断请求是否回来，再到请求中找到defaultselectId的对应的地址信息
      return userAddressList
        ? userAddressList.find((address) => address.id === defaultselectId)
        : [];
    },
  },
  async mounted() {
    ...
    this.defaultselectId = this.trade.userAddressList.find(
      (user) => user.isDefault === "1"
    ).id;
  },
};
</script>
